...
public class DinnerModel implements IDinnerModel{
    ...
    int nGuests = 0; //number of guests

    Set<Dish> menu = new HashSet<Dish>(); //set with selected dishes for the menu

	Set<Dish> dishes = new HashSet<Dish>(); //guess this is ALL dishes available (?)
    ...
    public Set<Dish> getFullMenu() //ok?
    {
        return menu;
    }

    public Set<Ingredient> getAllIngredients() //ok?
    {
    ...
    public float getTotalMenuPrice() //ok?
    {
        float price = 0;
        for(Dish d : menu){
            for(Ingredient i : d.getIngredients())
    ...
    public void addDishToMenu(Dish dish) //ok
    {
        menu.add(dish);
    }

    public Dish getSelectedDish(int type) //ok???
    {
        Dish result = new Dish("",0,"","");
        for(Dish d : menu){
            if(d.getType() == type){
                result = d;
            }
    ...
    public void removeDishFromMenu(Dish dish) //ok
    {
        menu.remove(dish);
    }

    public int getNumberOfGuests() //ok
    {
        return nGuests;
    }

    public void setNumberOfGuests(int numberOfGuests) //ok
    {
